...
namespace KatlaSport.Services.HiveManagement
{
    ...
    public class HiveSectionService : IHiveSectionService
    {
        ...
        {
            var dbHiveSections = await _context.Sections.Where(c => hiveSectionId == c.Id).ToArrayAsync().ConfigureAwait(false);
        ...
        {
            var dbHiveSections = await _context.Sections.Where(
                p => p.Code == updateRequest.Code && p.Id != hiveSectionId).ToArrayAsync();
            if (dbHiveSections.Length > 0)
            {
                throw new RequestedResourceHasConflictException("code");
            }
